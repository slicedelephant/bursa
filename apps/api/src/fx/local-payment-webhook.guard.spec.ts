...
const SECRET = 'lpwhsec_test';
const BODY = '{"depositRef":"ref_1","status":"SUCCEEDED"}';
...
function guardWith(secret: string | undefined): LocalPaymentWebhookGuard {
  const config = { get: () => secret } as unknown as ConfigService;
  return new LocalPaymentWebhookGuard(config);
}

describe('LocalPaymentWebhookGuard (E20)', () => {
  const now = Math.floor(Date.now() / 1000);

  it('admits a correctly signed raw body', () => {
    const ctx = context({
      headers: { 'x-local-payment-signature': header(BODY, now) },
      rawBody: BODY,
    });
  ...
  it('falls back to the parsed body when no rawBody is present', () => {
    const body = { depositRef: 'ref_1', status: 'FAILED' };
    const serialized = JSON.stringify(body);
    const ctx = context({
      headers: { 'x-local-payment-signature': header(serialized, now) },
      body,
    });
    expect(guardWith(SECRET).canActivate(ctx)).toBe(true);
  });
