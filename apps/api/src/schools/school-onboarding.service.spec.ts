...
function buildPrisma(overrides: Record<string, unknown> = {}) {
  const school = {
    findUnique: jest.fn(),
    update: jest.fn().mockImplementation(({ data }) => Promise.resolve({ id: 's1', name: 'ESMT', ...data })),
  };
...
function makeService(prisma: ReturnType<typeof buildPrisma>) {
  const esign = new MockEsignatureProvider(() => new Date('2026-06-29T10:00:00.000Z'));
...
describe('SchoolOnboardingService', () => {
  it('saves payout data and advances NOT_STARTED to IN_PROGRESS', async () => {
    ...
    prisma.school.findUnique.mockResolvedValue({ id: 's1', name: 'ESMT', onboardingStatus: 'NOT_STARTED', bic: null });
  ...
  it('keeps the status (and existing BIC) when payout is re-saved after activation', async () => {
    const prisma = buildPrisma();
    prisma.school.findUnique.mockResolvedValue({ id: 's1', name: 'ESMT', onboardingStatus: 'ACTIVE', bic: 'EXISTINGBIC' });
    const updated = await makeService(prisma).savePayout('s1', { ...fullPayoutDto, bic: undefined });
    expect(updated.onboardingStatus).toBe('ACTIVE');
    expect(updated.bic).toBe('EXISTINGBIC');
  });

  it('rejects an expired token', async () => {
    const created = createOnboardingToken({ now: new Date('2020-01-01T00:00:00.000Z'), ttlMs: 1000 });
    const prisma = buildPrisma();
    ...
    await expect(makeService(prisma).getOnboardingByToken(created.token)).rejects.toMatchObject({
  ...
  it('throws when the school does not exist', async () => {
    ...
    await expect(makeService(prisma).savePayout('missing', fullPayoutDto)).rejects.toMatchObject({
  ...
  it('blocks signing when payout data is incomplete', async () => {
    const prisma = buildPrisma();
    prisma.school.findUnique.mockResolvedValue({ id: 's1', name: 'ESMT', onboardingStatus: 'IN_PROGRESS' });
    await expect(makeService(prisma).signAgreement('s1', { signerName: 'Jane' })).rejects.toMatchObject({
  ...
  it('signs the agreement, activates the school and verifies payout', async () => {
    const prisma = buildPrisma();
    prisma.school.findUnique.mockResolvedValue({ id: 's1', name: 'ESMT', onboardingStatus: 'IN_PROGRESS', ...fullPayoutDto });
    const updated = await makeService(prisma).signAgreement('s1', { signerName: 'Jane Bursar' });
    expect(updated.onboardingStatus).toBe('ACTIVE');
  ...
  it('reads onboarding state from a valid token and rejects an invalid one', async () => {
    ...
    prisma.schoolOnboardingToken.findUnique.mockResolvedValue({
      ...
      expiresAt: created.expiresAt,
      usedAt: null,
      school: { id: 's1', name: 'ESMT', country: 'Germany', onboardingStatus: 'NOT_STARTED' },
    });
    ...
    await expect(makeService(prisma).getOnboardingByToken('bogus')).rejects.toMatchObject({
      response: { code: 'INVALID_TOKEN' },
    });
  });
