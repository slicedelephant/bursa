...
describe('validateFormSchema', () => {
  ...
  it('flags duplicate field keys', () => {
    ...
    expect(res.valid).toBe(false);
    expect(res.errors.some((e) => e.includes('Duplicate field key: why'))).toBe(true);
  });
  ...
  it('requires options on a SELECT field', () => {
    ...
    expect(res.valid).toBe(false);
    expect(res.errors.some((e) => e.includes('needs at least one option'))).toBe(true);
  });

  it('rejects invalid field keys', () => {
    const res = validateFormSchema([{ fieldKey: '1bad', label: 'X', type: 'TEXT' }]);
    expect(res.valid).toBe(false);
    expect(res.errors.some((e) => e.includes('Invalid field key'))).toBe(true);
  });

  it('rejects a missing label', () => {
    const res = validateFormSchema([{ fieldKey: 'ok', label: '  ', type: 'TEXT' }]);
    expect(res.valid).toBe(false);
  ...
  it('rejects a negative rubric weight', () => {
    ...
    expect(res.valid).toBe(false);
    expect(res.errors.some((e) => e.includes('negative rubric weight'))).toBe(true);
  });

  it('rejects a conditional reference to an unknown field', () => {
    const res = validateFormSchema([
      { fieldKey: 'a', label: 'A', type: 'TEXT', showIfFieldId: 'ghost', showIfValue: 'x' },
    ]);
    expect(res.valid).toBe(false);
    expect(res.errors.some((e) => e.includes('unknown showIf field ghost'))).toBe(true);
  });

  it('rejects a field depending on itself', () => {
    const res = validateFormSchema([
      { fieldKey: 'a', label: 'A', type: 'TEXT', showIfFieldId: 'a', showIfValue: 'x' },
    ]);
    expect(res.valid).toBe(false);
