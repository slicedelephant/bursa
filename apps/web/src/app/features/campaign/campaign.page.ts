  ...
  template: `
    <section class="mx-auto max-w-6xl px-4 py-10">
      @if (loading()) {
        ...
        <div class="mx-auto max-w-md rounded-2xl bg-white p-8 text-center shadow-card ring-1 ring-black/5">
        ...
        <a routerLink="/campaigns" class="inline-flex items-center gap-1 text-sm text-slate2 hover:text-ink">
        ...
        <div class="mt-4 grid gap-8 lg:grid-cols-3">
          ...
          <div class="space-y-8 lg:col-span-2">
            <div class="relative h-64 overflow-hidden rounded-2xl bg-gradient-to-br from-brand-green/20 to-brand-blue/20 sm:h-80">
              @if (c.photoUrl) {
                <img [src]="c.photoUrl" [alt]="c.studentName" class="h-full w-full object-cover" />
              } @else {
                <div class="flex h-full w-full items-center justify-center text-6xl font-bold text-brand-green">
                  {{ initials(c.studentName) }}
                </div>
              }
              @if (c.status === 'FUNDED' || c.status === 'DISBURSED') {
                <span class="absolute left-4 top-4 rounded-full bg-brand-green px-3 py-1 text-xs font-semibold text-white">
            ...
            <div class="space-y-3">
              <div class="flex flex-wrap items-center justify-between gap-2">
                <p class="text-sm font-medium text-slate2">{{ c.studentName }} · {{ c.studentCountry }}</p>
                <app-verified-badge [verified]="c.verified" />
              </div>

              <h1 class="font-display text-3xl font-semibold leading-tight text-ink">{{ c.title }}</h1>
            ...
            <div class="space-y-6 lg:sticky lg:top-6">
              <div class="rounded-2xl bg-white p-6 shadow-card ring-1 ring-black/5">
                ...
                <div class="mt-3 flex items-baseline justify-between">
                  <span class="font-display text-2xl font-semibold text-ink">{{ c.raisedCents | money }}</span>
                  <span class="text-sm text-slate2">raised of {{ c.goalCents | money }}</span>
                </div>
                <p class="mt-1 text-sm text-slate2">{{ c.donorCount }} supporter{{ c.donorCount === 1 ? '' : 's' }}</p>
